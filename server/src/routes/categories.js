...
categoriesRouter.get("/", async (req, res) => {
  res.json({ ok: true, categories: await listCategories(req.user.userId) });
});
...
categoriesRouter.post("/", async (req, res) => {
  ...
  const category = await addCategory(req.user.userId, {
...
categoriesRouter.put("/:id", async (req, res) => {
  const err = validateCategory(req.body);
  if (err) return res.status(400).json({ ok: false, error: err });
  const updated = await updateCategory(req.user.userId, req.params.id, {
    name: req.body.name.trim(),
    color: req.body.color,
    icon: req.body.icon || "",
...
categoriesRouter.delete("/:id", async (req, res) => {
  const ok = await deleteCategory(req.user.userId, req.params.id);
