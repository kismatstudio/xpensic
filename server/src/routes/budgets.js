...
import { getBudgets, setBudgets, getSettings, setSettings } from "../d1.js";
...
budgetsRouter.get("/", async (req, res) => {
  res.json({ ok: true, budgets: await getBudgets(req.user.userId) });
});
...
budgetsRouter.put("/", async (req, res) => {
  ...
  await setBudgets(req.user.userId, body);
  return res.json({ ok: true, budgets: await getBudgets(req.user.userId) });
});
...
settingsRouter.put("/", async (req, res) => {
  ...
  const settings = await setSettings(req.user.userId, patch);
  return res.json({ ok: true, settings });
});